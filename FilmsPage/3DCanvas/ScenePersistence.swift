...
final class ScenePersistenceService {
    ...
    func load(into vc: CanvasViewController, sceneID: UUID) async {
        ...
        // Phase 8b – BUG 5 FIX: Restore rotation arc visuals.
        // clearSceneState() clears activeRotationArcs; without this phase the arcs
        // are permanently invisible after reload even though the clips are intact.
        let rotationClips = vc.timeline.clips.filter { $0.track == .rotation }
        for (index, clip) in rotationClips.enumerated() {
            if index > 0 {
                try? await Task.sleep(nanoseconds: 16_000_000) // ~1 frame @ 60 fps
            }
            guard let anchor = vc.mainAnchor,
                  let entity = anchor.findEntity(named: clip.entityName) else { continue }
