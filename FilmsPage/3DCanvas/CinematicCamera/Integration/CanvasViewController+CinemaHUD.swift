...
extension CanvasViewController {
    ...
    private func presentLookPicker() {
        ...
        picker.onIntensityChanged = { [weak self] intensity in
            if let camRoot = self?.cameraToVisualMap[self?.activeCamera ?? PerspectiveCamera()],
               var lookComp = camRoot.components[CineLookComponent.self] {
                lookComp.intensity = intensity
                camRoot.components.set(lookComp)
            }
        }
